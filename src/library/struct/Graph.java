...
/*
 * // Used to signal violations of preconditions for // various shortest path algorithms. public class GraphException extends RuntimeException { public
 * GraphException(String name) { super(name); } }
 */
/*
 * // Represents an edge in the graph. class Edge { public Vertex dest; // Second vertex in Edge public double cost; // Edge cost
 * 
 * public Edge(Vertex d, double c) { dest = d; cost = c; } }
 */

/*
 * // Represents an entry in the priority queue for Dijkstra's algorithm. class Path implements Comparable<Path> { public Vertex dest; // w public double cost;
 * // d(w)
 * 
 * public Path(Vertex d, double c) { dest = d; cost = c; }
 * 
 * @Override public int compareTo(Path rhs) { double otherCost = rhs.cost;
 * 
 * return cost < otherCost ? -1 : cost > otherCost ? 1 : 0; } }
 */

/*
 * // Represents a vertex in the graph. class Vertex { public String name; // Vertex name public List<Edge> adj; // Adjacent vertices public double dist; //
 * Cost public Vertex prev; // Previous vertex on shortest path public int scratch; // Extra variable used in algorithm
 * 
...
public class Graph {
  ...
  public void dijkstra2(String startName) {
    ...
    clearAll();
    start.pairingHeapEntry = pq.insert(new Path(start, 0));
    start.dist = 0;
    ...
    while (!pq.isEmpty()) {
      ...
      for (Edge e : v.adj) {
        ...
        if (w.dist > v.dist + cvw) {
          ...
          if (w.pairingHeapEntry == null)
            w.pairingHeapEntry = pq.insert(newVal);
          else
            pq.decreaseKey(w.pairingHeapEntry, newVal);
        }
      }
    }
  }
