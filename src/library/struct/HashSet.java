...
public class HashSet<AnyType> extends AbstractCollection<AnyType> implements Set<AnyType> {
  ...
  /**
   * This method is not part of standard Java. Like contains, it checks if x is in the set. If it is, it returns the reference to the matching object; otherwise
   * it returns null.
   * 
   * @param x the object to search for.
   * @return if contains(x) is false, the return value is null; otherwise, the return value is the object that causes contains(x) to return true.
   */
  @Override
  ...
  /**
   * This is the implementation of the HashSetIterator. It maintains a notion of a current position and of course the implicit reference to the HashSet.
   */
