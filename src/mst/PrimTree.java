...
public final class PrimTree<T> {
  ...
  public static <T> UndirectedGraph<T> mst(UndirectedGraph<T> graph) {
    ...
    RedBlackBST<T> pq = new RedBlackBST<T>();
     ...
     */
    Map<T, RedBlackBST.Node<T>> entries = new HashMap<T, RedBlackBST.Node<T>>();
   ...
   */
  private static <T> void addOutgoingEdges(T node, UndirectedGraph<T> graph, RedBlackBST<T> pq, UndirectedGraph<T> result, Map<T, RedBlackBST.Node<T>> entries) {
    ...
    for (Map.Entry<T, Double> arc : graph.edgesFrom(node).entrySet()) {
      ...
      if ( !entries.containsKey( arc.getKey() ) ) { // Case 2
        //entries.put(arc.getKey(), pq.enqueue(arc.getKey(), arc.getValue()));
    	  entries.put( arc.getKey(), pq.put( arc.getKey(), arc.getValue() ) );
      }
